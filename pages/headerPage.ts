 ...
 *
 * Built from ARIA landmarks (`banner`, `navigation`) that hold for the large
 * majority of marketing and app sites, rather than any one site's markup.
 * If a target site doesn't expose these landmarks, narrow `header`/`nav`
 * here — every test and other page object keeps working unchanged.
 */
export class HeaderPage {
  constructor(private readonly page: Page) {}

  readonly header = this.page.getByRole("banner");
  readonly nav = this.header.getByRole("navigation").first();
  /** The site logo/home link — assumed to be the first link in the header. */
  readonly logo = this.header.getByRole("link").first();
  /** The collapsed-nav toggle shown on narrow viewports. Matches common naming: "menu", "nav", "hamburger". */
  readonly mobileMenuToggle = this.header.getByRole("button", { name: /menu|nav|hamburger/i });
